...
def deposit(req: DepositRequest, identity=Depends(get_current_identity), db: Session = Depends(get_session)):
    if not _has_permission_identity(identity, "deposit"):
        raise HTTPException(status_code=403, detail="API key missing deposit permission")
    ...
    return DepositInitOut(reference=out["reference"], authorization_url=out["authorization_url"])
...
def deposit_status(reference: str, db: Session = Depends(get_session), identity=Depends(get_current_identity)):
    if not _has_permission_identity(identity, "read") and not hasattr(identity, "email"):
    ...
    return DepositStatusOut(reference=data["reference"], status=data["status"], amount=data["amount"])
...
def transfer(req: TransferRequest, db: Session = Depends(get_session), identity=Depends(get_current_identity)):
    if not _has_permission_identity(identity, "transfer"):
        raise HTTPException(status_code=403, detail="API key missing transfer permission")
...
def transactions(db: Session = Depends(get_session), identity=Depends(get_current_identity)):
    if not _has_permission_identity(identity, "read"):
        raise HTTPException(status_code=403, detail="API key missing read permission")
    user_id = _identity_to_user_id(identity)
    txs = service.list_transactions_for_user(db, user_id)
    out_list = [
        {"type": t.transaction_type, "amount": float(t.amount), "status": t.transaction_status}
        for t in txs
    ]
