...
class WalletCRUD:
    ...
    def get_wallet_by_user(self, db: Session, user_id: str) -> Optional[Wallet]:
        return (
            db.query(self.wallet_model)
            .filter(self.wallet_model.walletuser_id == user_id)
            .first()
        )
    ...
    def get_wallet_by_number(self, db: Session, wallet_number: int) -> Optional[Wallet]:
        return (
            db.query(self.wallet_model)
            .filter(self.wallet_model.wallet_number == wallet_number)
            .first()
        )
    ...
    def get_transaction_by_reference(
        ...
        return (
            db.query(self.tx_model).filter(self.tx_model.reference == reference).first()
        )
    ...
    def list_transactions_for_user(
        ...
        return (
            db.query(self.tx_model)
            .filter(self.tx_model.walletuser_id == user_id)
            .order_by(self.tx_model.id.desc())
            .all()
        )
