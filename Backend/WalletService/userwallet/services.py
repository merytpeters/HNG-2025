...
class WalletService(WalletCRUD):
    ...
    def handle_webhook(self, db: Session, payload: Dict[str, Any]) -> Dict[str, Any]:
        with db.begin():
            ...
            # ensure static type checkers treat transaction_status as a concrete value
            if cast(str, tx.transaction_status) == TransactionStatus.SUCCESS.value:
                return {"status": True}
    ...
    def verify_deposit_status(self, db: Session, reference: str):
        ...
        return {
            ...
            "amount": float(cast(Any, tx.amount)),
        }
    ...
    def get_balance_for_user(self, db: Session, user_id: str) -> float:
        ...
        return float(cast(Any, wallet.balance))
